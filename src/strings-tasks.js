...
function sumOfCodes(str) {
  const result = str
    ? str
        .split('')
        .map((a, index) => str.charCodeAt(index))
...
function findLongestWord(sentence) {
  let word = '';
  sentence.split(' ').forEach((a) => {
    if (word.length < a.length) word = a;
  });
  return word;
}
...
function invertCase(str) {
  return str
    .split('')
    .map((a) => (a.toLowerCase() === a ? a.toUpperCase() : a.toLowerCase()))
    .join('');
}
...
function encodeToRot13(str) {
  const newstr = str
    ...
    .map((a) => {
      ...
      if (code > 64 && code < 91) {
        return String.fromCodePoint(((code - 65 + 13) % 26) + 65);
      }
      if (code > 96 && code < 123) {
        return String.fromCodePoint(((code - 97 + 13) % 26) + 97);
      }
      return a;
    })
    .join('');
